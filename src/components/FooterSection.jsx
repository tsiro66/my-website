import React from 'react';

const FooterSection = () => {
  const currentProjects = [
    { name: 'E-commerce Platform', tech: 'Next.js', status: 'In Progress' },
    { name: 'SaaS Dashboard', tech: 'React', status: 'Planning' },
    { name: 'Portfolio Site', tech: 'Vite', status: 'Completed' }
  ];

  return (
    <section className="min-h-screen bg-black flex items-center justify-center snap-start relative overflow-hidden">
      {/* Static gradient orb */}
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/6 -translate-y-1/6 w-96 h-96 bg-purple-600/20 rounded-full blur-3xl"></div>

      <div className="container mx-auto px-4 sm:px-6 md:px-12 lg:px-20 xl:px-32 relative z-10">
        <div className="max-w-6xl mx-auto">
          {/* Main CTA */}
          <div className="text-center mb-16">
            <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6">
              Έτοιμος να ξεκινήσουμε;
            </h2>
            <p className="text-xl md:text-2xl text-gray-400">
              Ας μετατρέψουμε την ιδέα σας σε πραγματικότητα
            </p>
          </div>

          {/* Info Grid */}
          <div className="grid md:grid-cols-3 gap-8 mb-16">
            {/* Availability */}
            <div className="bg-gray-900/50 backdrop-blur-sm rounded-xl p-6 border border-gray-800">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-white font-medium">Διαθεσιμότητα</h3>
                <div className="w-3 h-3 bg-green-500 rounded-full animate-pulse"></div>
              </div>
              <p className="text-gray-400 text-sm mb-2">Currently available for</p>
              <p className="text-white">Freelance Projects</p>
            </div>

            {/* Location */}
            <div className="bg-gray-900/50 backdrop-blur-sm rounded-xl p-6 border border-gray-800">
              <h3 className="text-white font-medium mb-4">Τοποθεσία</h3>
              <p className="text-gray-400 text-sm mb-2">Based in</p>
              <p className="text-white">Athens, Greece</p>
            </div>

            {/* Current Focus */}
            <div className="bg-gray-900/50 backdrop-blur-sm rounded-xl p-6 border border-gray-800">
              <h3 className="text-white font-medium mb-4">Τρέχοντα Projects</h3>
              <div className="space-y-2">
                {currentProjects.map((project, index) => (
                  <div key={index} className="flex items-center justify-between">
                    <span className="text-gray-400 text-sm">{project.name}</span>
                    <span className={`text-xs px-2 py-1 rounded-full ${
                      project.status === 'Completed' ? 'bg-green-500/20 text-green-400' :
                      project.status === 'In Progress' ? 'bg-yellow-500/20 text-yellow-400' :
                      'bg-purple-500/20 text-purple-400'
                    }`}>
                      {project.status}
                    </span>
                  </div>
                ))}
              </div>
            </div>
          </div>

          {/* Quick Links */}
          <div className="flex flex-wrap justify-center gap-6 text-gray-400">
            <a href="https://github.com/tsiro66" target="_blank" rel="noopener noreferrer" 
               className="hover:text-white transition-colors flex items-center gap-2">
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
              </svg>
              GitHub
            </a>
            <a href="https://www.linkedin.com/in/thodoris-tsironis-173b1b24a/" target="_blank" rel="noopener noreferrer" 
               className="hover:text-white transition-colors flex items-center gap-2">
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
              </svg>
              LinkedIn
            </a>
            <span className="hover:text-white transition-colors">
              tsiro.thodoris@gmail.com
            </span>
            <span className="hover:text-white transition-colors">
              +30 698 782 8639
            </span>
          </div>
        </div>
      </div>

      {/* Minimal copyright */}
      <div className="absolute bottom-8 left-0 right-0 text-center text-gray-600 text-xs">
        © {new Date().getFullYear()} Θοδωρής Τσιρώνης
      </div>
    </section>
  );
};

export default FooterSection;